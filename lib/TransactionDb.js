...
var genesisTXID = 'f1cde99d4ccfd3bab235232b48261dc907a4bcc5a0231f1f2dc47a6c0519aa0c';
...
var encodedData = require('soop').load('iadixcore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('iadixcore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('iadixcore/lib/Address', {
...
TransactionDb.prototype.fromShortAddr = function(shortaddr, cb) {
  var addr;
  var k = ADDR_PREFIX + shortaddr;
  ...
  db.createReadStream({
    ...
    .on('data', function(data) {
        addr = data.key.split('-')[1];
    })
    .on('error', cb)
    .on('end', function() {
        if(addr) {
            return cb(false, addr);
        } else {
            return cb("No such address");
        }
    });
};

TransactionDb.prototype.fromShortTx = function(shorttx, cb) {
  var txid;
  var k = OUTS_PREFIX + shorttx;
  ...
  db.createReadStream({
    ...
    .on('data', function(data) {
        txid = data.key.split('-')[1];
    })
    .on('error', cb)
    .on('end', function() {
        if(txid) {
            return cb(false, txid);
        } else {
            return cb("No such transaction");
        }
    });
};
